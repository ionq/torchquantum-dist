...
def sampler_nondiff_exact(
    ...
    gpu_probs = maybe_full_tensor(gpu_probs).view(
        (orig_shape_local[0], -1)
    )  # all gather  # (b, n_gpus)
...
def measure_allZ(
    ...
    state_mag = (states**2).sum(
        -1
    )  # PauliZ hardocded here; no rotation before grabbing probabilities
    ...
    if postselect_cond:
        ...
        for i in range(len(post_wires)):
            ...
            if (
                wire_info[1].item() == -1
            ):  # Ungrouped/sharded qubits are easiest to handle
                slice_idx = (
    ...
    ].tolist()  # Unreduced dim remains the same as qubits are shunted around
    shard_reduced_groupings = groupings.detach().clone()
    for wire in grouped_wires:
        shard_reduced_state_mag, shard_reduced_groupings = q_device.interchange_qubits(
    ...
    y = probs @ torch.tensor([1.0, -1.0], device=probs.device)  # hardcoded PauliZ
